/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ScheduleIntervalWhereInput } from "./ScheduleIntervalWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { ScheduleIntervalOrderByInput } from "./ScheduleIntervalOrderByInput";

@ArgsType()
class ScheduleIntervalFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => ScheduleIntervalWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => ScheduleIntervalWhereInput, { nullable: true })
  @Type(() => ScheduleIntervalWhereInput)
  where?: ScheduleIntervalWhereInput;

  @ApiProperty({
    required: false,
    type: [ScheduleIntervalOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [ScheduleIntervalOrderByInput], { nullable: true })
  @Type(() => ScheduleIntervalOrderByInput)
  orderBy?: Array<ScheduleIntervalOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { ScheduleIntervalFindManyArgs as ScheduleIntervalFindManyArgs };
